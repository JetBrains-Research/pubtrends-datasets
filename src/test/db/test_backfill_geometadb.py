...
class TestGEOmetadbBackfiller(unittest.TestCase):
    ...
    @staticmethod
    def _make_response(gse_accession: str, valid_body=True):
    ...
    def test_backfill_geometadb_success(self, mock_get: Mock, mock_get_accessions: Mock):
        mock_get.return_value.__aenter__.return_value = self._make_response("GSE000000")
        mock_get_accessions.return_value = ["GSE000000"]
        ...
        self.assertEqual(datasets[0].gse, "GSE000000")
        self.repository.save_gses_async.assert_called_once()

    @patch("src.db.backfill_geometadb.get_geo_ids")
    @patch("aiohttp.ClientSession.get")
    def test_backfill_geometadb_download_failure(self, mock_get: Mock, mock_get_accessions: Mock):
        mock_get.side_effect = asyncio.TimeoutError("Download failed")
        mock_get_accessions.return_value = ["GSE000000"]
        start_date, end_date = datetime.datetime(2025, 1, 1), datetime.datetime(2025, 1, 2)
        self.assertRaises(asyncio.TimeoutError, self.backfiller.backfill_geometadb, start_date, end_date)
        mock_get_accessions.assert_called_once()
        self.assertEqual(mock_get.call_count, RETRY_ATTEMPTS)

    @patch("src.db.backfill_geometadb.get_geo_ids")
    @patch("aiohttp.ClientSession.get")
    def test_backfill_geometadb_invalid_gzip(self, mock_get: Mock, mock_get_accessions: Mock):
        mock_get.return_value.__aenter__.return_value = self._make_response("GSE000000", valid_body=False)
        mock_get_accessions.return_value = ["GSE000000"]
        start_date, end_date = datetime.datetime(2025, 1, 1), datetime.datetime(2025, 1, 2)
        self.assertRaises(gzip.BadGzipFile, self.backfiller.backfill_geometadb, start_date, end_date)
        mock_get_accessions.assert_called_once()
        self.assertEqual(mock_get.call_count, RETRY_ATTEMPTS)
